...
class GFPGANModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        # ----------- define net_d ----------- #
        self.net_d = build_network(self.opt['network_d']) # self.net_g.net.decoder #
        ...
        # self.net_g_ema = build_network(self.opt['network_g']).to(self.device) #self.net_g 
        # # load pretrained model
        # load_path = self.opt['path'].get('pretrain_network_g', None)
        # if load_path is not None:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            # gan loss
            self.output = torch.nn.functional.interpolate(self.output, size=(512,512) , mode='bilinear')
            with torch.no_grad():
        ...
        self.gt = torch.nn.functional.interpolate(self.gt, size=(512,512) , mode='bilinear')
    ...
    def save(self, epoch, current_iter):
        ...
        self.save_network([self.net_g], 'net_g', current_iter, param_key=['params', 'params_ema'])
